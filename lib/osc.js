...
var Server = function(port, host) {
    ...
    function address_matches(address, test) {
    ...
    this._sock.on('message', function (msg, rinfo) {
        ...
        for (var c=0; c<_callbacks.length; c++) {
            ...
            if (address_matches(decoded[0], _callbacks[c].address)) {
    ...
    this.removeMsgHandler = function(address) {
        for (var c=0; c<_callbacks.length; c++) {
            // if they're matching strings or matching regex
            if (address_matches(address, _callbacks[c].address)) {
                delete _callbacks[c];
            }
        }
    };
